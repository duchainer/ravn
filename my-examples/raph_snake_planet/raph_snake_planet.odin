...
rand_dir :: proc() -> [3]f32 {
	return linalg.normalize0(
		[3]f32{rand.float32() * 2.0 - 1.0, rand.float32() * 2.0 - 1.0, rand.float32() * 2.0 - 1.0},
	)
}
...
_init :: proc() {
	...
	state.death_sound =
		rv.create_sound_resource_encoded(
			"death",
			#load("../../examples/data/snake_death_sound.wav"),
		) or_else panic("create_sound_resource_encoded")
	state.berry_sound =
		rv.create_sound_resource_encoded(
			"death",
			#load("../../examples/data/snake_powerup_sound.wav"),
		) or_else panic("create_sound_resource_encoded")
...
_update :: proc(hot_state: rawptr) -> (data_ptr: rawptr) {
	...
	_update_game :: proc(delta: f32) {
		...
		for &seg, i in snake.segments[:snake.num_segments] {
			...
			seg.pos =
				prev + (linalg.normalize0(seg.pos - prev) * PLANET_SIZE) * (0.15 / PLANET_SIZE)
		}
		...
		target_rot := linalg.quaternion_from_forward_and_up_f32(state.cam.pos, cam_rot_mat[1])
		state.cam.rot = target_rot
		// JUICE on berry collect:  Widen FOV for 0.5s
		state.cam.fov = rv.lexp(
			state.cam.fov,
			rv.deg(65 + rv.remap_clamped(state.berry_timer, 0, 0.5, 7, 0)),
			rate = delta * 15,
		)
	...
	if state.screen == .Game || state.screen == .Death {
		...
		for seg, i in snake.segments[:snake.num_segments] {
			rv.draw_mesh(
				handle = sphere,
				pos = seg.pos *
				(1.0 + 0.025 * rv.nsin(f32(i) * 0.21 - state.animation_clock_time)),
				scale = 0.15,
		...
		for obst in state.obsts[:state.num_obsts] {
			...
			rv.draw_mesh(sphere, obst.pos, scale = obst.radius, col = color)
			color = [4]f32{0.2, 0.7, 0.3, 1}
			rv.draw_mesh(sphere, obst.pos * 1.1, scale = obst.radius, col = color)
		}

		rv.draw_mesh(
			sphere,
			state.berry.pos * 1.1,
			scale = 0.2 + 0.05 * rv.nsin(rv.get_time() * 2),
			col = rv.RED,
		)
	}
	...
	case .Game:
		{
			...
			rv.draw_text("Use WASD to move", {screen_size.x * 0.5, 24, 0.1}, anchor = 0, scale = 2)
		}
	...
	case .Menu:
		{
			char_sprites := rv.draw_text(
				...
				{
					screen_size.x * 0.5,
					screen_size.y * 0.5 + math.sin_f32(rv.get_time() * 2) * 4,
					0,
				},
				anchor = 0,
				scale = 4,
			...
			rv.draw_text(
				ufmt.tprintf("HIGHSCORE %i", state.max_score),
				{screen_size.x * 0.5, screen_size.y * 0.65, 0.1},
				anchor = 0,
				scale = 2,
			)
			rv.draw_text(
				"Music by Nolram. Thank you!",
				{screen_size.x * 0.5, screen_size.y - 32, 0.1},
				anchor = 0,
				scale = 2,
				col = rv.hex_color(0x06e4c2),
			)
